...
### Quickstart (WSL + 1x RTX 5090)

If you're running locally in WSL with a single RTX 5090, this repo includes helper scripts with safer single-GPU defaults.

```bash
cd parameter-golf
chmod +x setup_5090_wsl.sh run_5090_wsl.sh
./setup_5090_wsl.sh
./run_5090_wsl.sh
```

The launcher keeps the baseline code path but reduces default local memory pressure:

- `TRAIN_BATCH_TOKENS=131072` (instead of 524288)
- `VAL_BATCH_SIZE=131072` (instead of 524288)
- `nproc_per_node=1`

Override any value as needed, for example:

```bash
TRAIN_BATCH_TOKENS=196608 MAX_WALLCLOCK_SECONDS=0 ./run_5090_wsl.sh
```
